...
- 현시점의 정보에 대한 후보군 계산
4. **은닉층**
- 업데이트 결과와 후보군 결과를 결합하여 현시점ㄴ의 은닉층 계산
---
...
- 주어진 데이터를 학습하여, 데이터 분포를 따르는 **유사한 데이터를 생성**하는 모델

┌ discriminative model (판별자 모델) : 입력 이미지(x)에서 특성 추출을 잘 수행해서 정답(y)를 정확히 분류한다.  
└ generative model (생성자 모델) : 판별자 모델에서 추출한 특성들의 조합을 이용하여 새로운 입력 이미지를 만들어 낸다. 
입력 이미지에 대한 데이터 분포 p(x)를 학습

--- 

### 생성 모델의 유형
**1. 명시적 방법 (explicit density)**
- 확률 변수 p(x) 정의
- 변형 오토인코더 (variational autoencoder)  

**2. 암시적 방법 (implicit density)**
- 확률 변수 p(x)에 대한 정의 없이 p(x)를 샘플링하여 사용
- GAN(Generative Adversarial Network)

---
### Autoencoder
- 입력을 출력으로 복사하는 신경망
- 은닉층의 노드 개수가 입력값보다 적음

1. **인코더 (인지 네트워크 / Recognition Network)** : 특성에 대한 학습 수행
2. **은닉층 (병목층)** : 모델의 뉴런 개수가 최소인 계층. 차원이 가장 낮은 입력 데이터의 압축 표현이 포함됨
3. **디코더 (생성 네트워크 / Generative Network)** : 병목층에서 압축된 데이터를 원래대로 재구성(reconstruction). 최대한 입력에 가까운 출력 생성
4. **손실 재구성** : 입력층과 출력층의 뉴런 개수가 동일하다는 것만 제외하면 일반적인 다층 퍼셉트론 (MLP)과 구조가 동일. 손실 함수는 입력과 출력의 차이로 계산

#### 오토인코더가 중요한 이유
1. 데이터 압축
2. 차원의 저주 예방 : 특성 개수를 줄여주기 때문에 데이터 차원이 감소하여 차원의 저주 피할 수 있음
3. 특성 추출 : 비지도 학습으로, 자동으로 중요한 특성을 찾아 준다.

#### 오토인코더에서 많이 사용되는 손실함수
1. 평균 제곱 오차 : 입력 값이 (0,1) 범위에 있지 않은 경우
